  ...
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      if (isRetryable && i < maxRetries - 1) {
        ...
        console.log(`Panda is busy or rate limited (${error.message?.includes("429") ? "429" : "503"}). Retrying in ${delay}ms... (Attempt ${i + 1}/${maxRetries})`);
...
export async function structureGoalFromAudio(audioBase64: string, mimeType: string, userContext?: UserContext): Promise<StructuredGoal> {
  ...
  try {
    let response;
    try {
      console.log("Attempting with best model: gemini-3.1-pro-preview");
      response = await generateWithModel("gemini-3.1-pro-preview");
    } catch (error: any) {
      const isQuotaError = error.message?.includes("429") || error.message?.includes("RESOURCE_EXHAUSTED");
      if (isQuotaError) {
        console.warn("Best model quota exceeded. Falling back to next available model: gemini-3-flash-preview");
